...
module FontelloRailsConverter
  class Cli
    ...
    def download
      ...
      File.write(@options[:zip_file], @fontello_api.download_zip_body)
    ...
    def convert_for_asset_pipeline(content)
      # asset URLs
      content.gsub!('../font/', "")
      content.gsub!(/url\(([^(]+)\)/) do |_m|
        ::Regexp.last_match(1).include?("application/octet-stream") ? "url(#{::Regexp.last_match(1)})" : "font-url(#{::Regexp.last_match(1)})"
      end
    end
    ...
    def convert_for_webpack(content)
      content.gsub!('../font/', "")
      content.gsub!(/url\(([^(]+)\)/) do |_m|
        replace = if ::Regexp.last_match(1)[0] == "'"
                    "'~#{::Regexp.last_match(1)[1..]}"
                  else
                    "~#{::Regexp.last_match(1)}"
                  end
        ::Regexp.last_match(1).include?("application/octet-stream") ? "url(#{::Regexp.last_match(1)})" : "url(#{replace})"
      end
    end
    ...
    def sass_enhance(content)
      ...
      content.gsub!(/^\.(icon-\S+:before) { (.+)$/) { |_m| "%#{::Regexp.last_match(1)} { @extend %icon-base; #{::Regexp.last_match(2)}" }
    ...
    def convert_icon_guide
      ...
      File.write(icon_guide_target_file, self.class.convert_icon_guide_html(content))
    ...
    def icon_guide_target_file
      @icon_guide_target_file ||= File.join(@options[:icon_guide_dir], "fontello-demo.html")
    end

    def stylesheet_file(postfix: '', extension: '.css')
      if fontello_name && @options[:stylesheet_dir] && !@options[:stylesheet_dir].empty?
    ...
    def fontello_name
      @fontello_name ||=
        if config_file_exists?
          ...
          name_from_config.nil? || name_from_config.empty? ? 'fontello' : name_from_config
        end
    end
  end
end
